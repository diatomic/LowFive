...
struct PointBlock
{
    ...
    // check if received data matches original
    void check_recvd_data(const diy::Master::ProxyWithLink& cp)
    {
        for (auto i = 0; i < grid.size(); ++i)
        {
            ...
            {
                fmt::print("Error: grid[{}] = {} but does not match read_grid[{}] = {}\n",
                        i, grid[i], i, recv_grid[i]);
                exit(0);
            }
        }
        fmt::print("[{}] Success: received grid matches original.\n", cp.gid());
    }
